...
**在编写任何代码之前**，必须向用户确认以下清单。全部项目确认后，才能进入 Phase 2。

```
□ PPT 用途：________________（答辩 / 汇报 / 路演 / 课程报告 / 其他）
□ 受众与氛围期望：________________（正式严肃 / 专业简洁 / 活泼创意 / 其他）
□ 内容来源：________________（论文路径 / 文档路径 / 数据 / 口头描述）
□ 页数预估：___ 页
□ 图片素材路径：________________，共 ___ 张
□ 需要突出展示的关键数据：________________
□ 是否需要演讲者备注：是 / 否
□ 输出路径：________________
□ 机构/品牌信息：________________（校名 / 公司名 / Logo 路径）
```

使用 `AskUserQuestion` 工具一次性提问，将相关问题分组为最多 4 个问题，每问 2-4 个选项加"其他"。

---

### Phase 2: 配色方案选择

**核心原则：不写死颜色。根据 Phase 1 确认的用途和受众，自主设计配色方案。**

决策维度：
- **用途** → 色调方向：学术答辩偏沉稳（深蓝/藏青/酒红），商业路演偏锐利（深灰+亮色点缀），创意汇报可活泼（高饱和点缀）
